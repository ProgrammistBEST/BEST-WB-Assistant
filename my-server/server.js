...
app.get('/kyz', async (req, res) => {
    ...
    try {
        ...
        // Выбираем доступную запись (статус Waiting или Comeback)
        const selectQuery = `
            SELECT id, Crypto, Model, Size 
            FROM ${tableName}
            WHERE Size = ? AND Brand = ? AND Model = ? AND Status IN ('Comeback', 'Waiting')
        `;
        const [rows] = await pool.execute(selectQuery, [size, brand, model]);
        ...
        // Обновляем статус записи по её id
        const updateQuery = `
            UPDATE ${tableName} 
            SET Status = 'Reserved'
            WHERE id = ? AND Status IN ('Comeback', 'Waiting')
        `;
        await pool.execute(updateQuery, [selectedId]);
